...
class DDoSDetector:
    def __init__(self, config: AppConfig):
        ...
        logger.info(f"Initializing DDoS Guard...")
        ...
        if not self.model_path.exists():
            ...
            logger.error("The package was built without model files or they are missing.")
            sys.exit(1)
    ...
    def _run_cicflowmeter_live(self):
        logger.info(f"Starting CICFlowMeter on interface {self.config.system.interface}...")
        ...
        cmd = [
            "cicflowmeter",
            "-i", self.config.system.interface,
            "-c", self.config.system.csv_output_path
        ]
        
        # Run in background, suppress standard output to keep CLI clean
        self.cic_process = subprocess.Popen(cmd, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
    ...
    def _run_cicflowmeter_pcap(self, pcap_path: str):
        ...
        cmd = [
            "cicflowmeter",
            "-f", pcap_path,
            "-c", self.config.system.test_mode_output
        ]
