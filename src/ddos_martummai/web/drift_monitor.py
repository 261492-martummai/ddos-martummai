...
WINDOW = 60
meta_prob_history = deque(maxlen=WINDOW)
pkt_rate_history = deque(maxlen=WINDOW)
byte_rate_history = deque(maxlen=WINDOW)

# การตั้งค่าสำหรับ Auto-Baseline
AUTO_BASELINE_THRESHOLD_MIN = 0.1
AUTO_BASELINE_THRESHOLD_MAX = 0.45
AUTO_BASELINE_DURATION = 600
...
def save_baseline():
        ...
        logger.info(f"[ACTION] New Baseline Saved: Packets={pkt_mean:.2f}/s, Bandwidth={byte_mean:.2f} B/s")
    ...
    if AUTO_BASELINE_THRESHOLD_MIN <= current_drift <= AUTO_BASELINE_THRESHOLD_MAX:
        if drift_stable_start_time is None:
            ...
            if elapsed >= AUTO_BASELINE_DURATION:
                logger.info(f"[AUTO-BASELINE] Drift stable for {AUTO_BASELINE_DURATION}s. Updating baseline...")
                save_baseline()
                drift_stable_start_time = None
    else:
